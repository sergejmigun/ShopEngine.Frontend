app.registerComponent('strings', 'Utils', [
    ...
    'Collections',
    'Lib.objects',
    function ($, collections, libObjects) {
        'use strict';
        return {
            ...
            format: function () {
                var regex = new RegExp("{-?[0-9]+}", "g"), args = collections.from(arguments).skip(1).toArray();
                return arguments[0].replace(regex, function (item) {
                    var intVal = parseInt(item.substring(1, item.length - 1), 10), replace;
                    if (intVal >= 0) {
                        replace = args[intVal];
                    }
                    else if (intVal === -1) {
            ...
            isNullOrWhiteSpace: function (str) {
                ...
                return !collections.from(str).any(function (symbol) {
                    return symbol !== ' ';
                });
            },
            concat: function () {
