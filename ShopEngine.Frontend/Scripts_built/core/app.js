class App {
    constructor() {
        this.modulesInitData = [];
        this.componentsInitData = {};
        this.contextObjects = {};
        this.dependenciesScope = [];
        this.componentsCache = {};
        this.ignoreParams = function (args) {
            return args;
        };
    }
    getDependenciesPath(fullName) {
        var path = '';
    ...
    getDependency(dependecyInfo) {
        var nameParts = dependecyInfo.split('.'), dependency = this;
    ...
    getDependencies(dependeciesInfo) {
    ...
    getInstance(initData, fullName) {
        ...
        var constructorFunc = initData[initData.length - 1], dependeciesInfo = [], i;
    ...
    getModuleInstance(moduleName, initData) {
        var moduleObj = this.getInstance(initData, moduleName), self = this;
        if (self.componentsInitData[moduleName]) {
            self.componentsInitData[moduleName].forEach(function (componentInitData) {
                var nsParts = componentInitData.moduleName.split('.'), obj = moduleObj.module, componentName = componentInitData.componentName, i;
    ...
    registerModule(moduleName, initData) {
        this.modulesInitData.push({
            moduleName: moduleName,
            initData: initData
        });
    }
    registerComponent(componentName, namespaceModuleName, initData) {
        if (!componentName) {
        ...
        var components, moduleName = namespaceModuleName.split('.')[0];
