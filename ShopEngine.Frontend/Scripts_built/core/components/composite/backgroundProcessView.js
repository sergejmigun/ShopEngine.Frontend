app.registerComponent('backgroundProcessView', 'Components', [
    ...
    'Shared',
    'Services',
    function ($, promise, objects, shared, services) {
        'use strict';
        return {
            init: function (model, containerData) {
                var template = {}, _$html = $('<div />'), _events = services.eventsInitializer.init(template, ['completed']);
                function initControl(success) {
                    var $wrapper = $('<div class="margin-b-20" />'), url = shared.systemUrls.getBackgroundProcessInfoApiUrl + '?processId=' + model.processId;
