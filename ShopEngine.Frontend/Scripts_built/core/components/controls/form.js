app.registerComponent('form', 'UI', [
    ...
    'UI.loader',
    function (promise, templates, objects, collections, containerHelper, controlValidation, errorHandler, controlsChangesDetectorFactory, loader) {
        'use strict';
        return {
            init: function (initData) {
                var form = {}, _validation = controlValidation.init({
                    validateOnChange: true
                }), _loader, _isReady = false, _controlsChangesDetector = controlsChangesDetectorFactory.init(), _controlsData = [], _controlPromises = [], _templatesData = [], _templatePromises = [], _serializationData = [];
                ...
                function displayServerError(objectsPath, model, message) {
                    ...
                    var control = findControlByModelProperty(model, objectsPath.name), modelProperty = model[objectsPath.name];
                    if (!control) {
                ...
                function getCustomSerializationInitFunc(controlPromise) {
                    return function (serializationRule) {
                ...
                function getValidationInitFunc(controlPromise) {
                    return function (validatorName, validator) {
                        controlPromise.then(function (control) {
                ...
                form.ready = function () {
                    ...
                    return promise.all(_controlPromises.concat(_templatePromises)).then(function () {
                        _isReady = true;
                    });
                };
