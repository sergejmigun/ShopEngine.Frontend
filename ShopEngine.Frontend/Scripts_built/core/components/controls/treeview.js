    ...
    function ($, promise, objects, collections, eventsInitializer, ui) {
        ...
        return {
            init: function (container, initData) {
                var control = {}, _$wrapper = $('<div class="tree-view" />'), _$nodesContainer, _nodesData = [], _events = eventsInitializer.init(control, ['renderCompleted', 'renderItemCompleted', 'onToggle']);
                ...
                function walkNodes(nodesData, action) {
                    ...
                    collections.foreach(nodesData, function (nodeData) {
                ...
                function buildNode(node, parentNodeData, index) {
                    ...
                    function initSelection(nodeData) {
                        ...
                        function selectChildNodes(currentNodeData) {
                            if (currentNodeData.nodesData) {
                                collections.foreach(currentNodeData.nodesData, function (childNodeData) {
                    ...
                    var nodesData = parentNodeData.nodesData, nodeData = {
                        node: node,
                    ...
                    var buildPromise = ui.renderItem(nodeData.element.find('.nodeContentArea'), container.ready(), nodeData.node, objects.tryGet(nodeData.node.data, 'text'), true);
