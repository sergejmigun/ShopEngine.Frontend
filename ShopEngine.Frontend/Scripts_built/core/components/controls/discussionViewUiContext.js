app.registerComponent('discussionViewUiContext', 'Services.discussionView', [
    ...
    'UI.fileUploader',
    'UI.modals',
    function ($, promise, collections, shared, lib, eventsInitializer, containerHelper, strings, dom, dates, popover, emotionsPicker, textBox, carousel, fileUploader, modals) {
        'use strict';
        return {
            init: function (container, initData) {
                var context = {}, _$wrapper = $('<div class="discussion-view-wrapper" />'), _$commentsWrapper = $('<div class="discussion-comments-wrapper" />'), _defaultAvatarImageSrc = '//a.disquscdn.com/1455048077/images/noavatar92.png', _currentTextSelectionComment, _replies, _localization = {
                ...
                function initAvatar(url, $avatarContainer) {
                    ...
                    initImage().then(function ($img) {
                        $avaWrapper.append($img);
                    });
                }
                function initInputBox($container, commentData, needCancel) {
                    var $formWrapper = $('<form />'), $inputWrapper = $('<div class="discussion-input-wrapper" />'), $errorWrapper = $('<div class="discussion-form-error-wrapper" />').hide(), $actionsWrapper = $('<div class="discussion-form-actions-wrapper" />'), inputBox = {}, inputBoxEvents = eventsInitializer.init(inputBox, ['submit', 'cancel']), validationTimeOuts = [];
                    function getContentLength(targetContent) {
                        return $('<div />').html(targetContent).text().length;
                    }
                    function initActions(editor) {
                        var $left = $('<div class="pull-left" />'), $right = $('<div class="pull-right" />');
                        function initSubmit() {
                        ...
                        function initEmotions() {
                            var $smile = $(emotionsPicker.getEmotionHtml('smile', 20)), $emotionsContainer = $('<div />');
                        ...
                        function initImageUploadings() {
                            var $imageLoad = $('<div class="discussion-image-load" />'), $imageFormContainer = $('<div class="discussion-image-box" />'), $imageFormUrl = $('<div><span>Enter an image url:</span><br />' +
                                '<div class="discussion-image-url-wrapper"><input type="hidden" /></div></div>'), $imageFormButton = $('<button class="discussion-button">Post</button>'), $imageFormUpload = $('<div><br><input type="hidden" /></div>'), $icon = $('<i class="glyphicon glyphicon-picture" />').attr('title', 'Load image'), $uploadImageCustomButton = $('<a><i class="glyphicon glyphicon-open" /> Upload image</a>');
                            ...
                            var tbInit = textBox.init(containerHelper.replace($imageFormUrl.find('input'), container.ready()), {}), uplInit = fileUploader.init(containerHelper.replace($imageFormUpload.find('input'), container.ready()), {
                                customButton: $uploadImageCustomButton,
                                showProgressBar: false,
                                showUploadedFiles: false,
                                uploadUrl: '/Help/Upload'
                            });
                            promise.all([tbInit, uplInit]).then(function (instances) {
                                var textBox = instances[0], uploader = instances[1];
                        ...
                        function initAttachmentUploadings() {
                            var $attachLoad = $('<div class="discussion-attachment-load" />').attr('title', 'Load attachments'), $loadedAttachments = $('<div class="discussion-loaded-attachments" />'), $loadedAttachmentsInner = $('<div />'), $hidden = $('<input type="hidden" />'), $icon = $('<i class="glyphicon glyphicon-paperclip" />');
                            ...
                            carousel.init(containerHelper.appendTo($loadedAttachmentsInner, container.ready()), {
                                ...
                                function addCarouselItem(attachment) {
                                    var $itemWrapper = $('<span class="discussion-loaded-attachment-item" />'), $link = $('<a />').text(attachment.title).attr('href', attachment.url), $remove = $('<i class="glyphicon glyphicon-remove-circle" />'), carouselItem = {
                                        content: $itemWrapper,
                                ...
                                fileUploader.init(containerHelper.replace($hidden, container.ready()), {
                                    ...
                                    uploader.onDone(function (data) {
                                        var fileName = data.files[0].name, attachment = {
                                            title: fileName,
                ...
                function initReply(reply) {
                    function initCommentActions() {
                        var $wrapper = $('<div class="discussion-actions" />'), needBullet;
                        ...
                        function initReplyAction() {
                            ...
                            $replyLink = $('<a href="#" class="discussion-reply" />').text(_localization.reply).click(function () {
                                var $replyInputContainer = $('<div class="discussion-reply-input-wrapper" />'), replyContent = null;
                        ...
                        function initEditAction() {
                            ...
                            var $bullet = checkBullets(), $editLink = $('<a href="#" class="discussion-edit" />').text(_localization.edit);
                            function toggle$Elements(action) {
                                $editLink[action]();
                                if ($bullet) {
                                    $bullet[action]();
                                }
                        ...
                        function initLikeAction() {
                            function initLikes() {
                                var $likeToggle = $('<span class="discussion-like-toggle" />'), $likesCount = $('<span />'), $likeIcon = $('<i />');
                            ...
                            function initThumbs() {
                                var $thumbsUp = $('<span class="discussion-thumbs-toggle discussion-thumbs-up" />'), $thumbsDown = $('<span class="discussion-thumbs-toggle discussion-thumbs-down" />');
                        ...
                        function initToggleRepliesAction() {
                            var $toggleReplies = $('<i class="glyphicon discussion-replies-toggle" />').hide(), $repliesCount = $('<span class="discussion-replies-count" />').hide();
                    ...
                    function initCommentInfo() {
                        var $wrapper = $('<div class="discussion-info" />'), $autnorLink = $('<a href="#" class="discussion-person" />'), $time = $('<span class="discussion-time" />');
                        function formatDate() {
                            if (initData.dateFormatter) {
                                $time.text(initData.dateFormatter(reply.date));
                                return;
                            }
                            var diff = Date.now() - reply.date.getTime(), diffSeconds = Math.abs(diff / 1000), text;
                            if (diffSeconds < 60) {
                        ...
                        if (reply.parent) {
                            var $replyTo = $('<span class="discussion-replied-to" />').append('<i class="glyphicon glyphicon-share-alt"></i>'), $replyToLink = $('<a href="#" class="discussion-person" />');
                            appendBullet($wrapper);
                    ...
                    function initRepliesContainer() {
                        ...
                        var currentDepth = 1, parent = reply.parent;
                        while (parent) {
                ...
                context.initMainInput = function () {
                    initInputBoxWithAvatar(_$wrapper, {}, false).then(function (inputBox) {
                        inputBox.submit(function () {
