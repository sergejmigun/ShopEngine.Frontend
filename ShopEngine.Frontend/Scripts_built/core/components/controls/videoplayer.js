app.registerComponent('videoPlayer', 'UI', [
    '$',
    'Promise',
    'Utils.objects',
    'Services.eventsInitializer',
    'Services.apiService',
    function ($, promise, objects, eventsInitializer, apiService) {
        'use strict';
        return {
            init: function (container, initData) {
                var control = {}, _mediaData = [], _currentMediaIndex, _$wrapper = $('<div class="player-container" />'), _$element, _events = eventsInitializer.init(control, ['playStart', 'playEnd']);
                ...
                function init(success) {
                    ...
                    if (initData.noSkin) {
                        ...
                        var playerId = 'videoPlayer_' + Date.now();
                        _$wrapper.attr('id', playerId);
                        apiService.getTemplateHtml('VideoPlayerTemplate').then(function (html) {
                            _$wrapper.html(html);
                            var $playPrev = _$wrapper.find('.jp-previous'), $playNext = _$wrapper.find('.jp-next'), playerOptions = {
                                useStateClassSkin: true,
                                cssSelectorAncestor: '#' + playerId
                            };
                            if (initData.palylist) {
                                _$wrapper.addClass('video-player-playlist');
                ...
                control.setMedia = function (title, poster, source, sourceFormat) {
                    if (!sourceFormat) {
                        sourceFormat = 'm4v';
                    }
                    var media = {
