app.registerComponent('barChart', 'UI', [
    ...
    'Utils.strings',
    function ($, promise, colors, collections, eventsInitializer, chartHelper, strings) {
        'use strict';
        return {
            init: function (container, initData) {
                var control = {}, _$wrapper = $('<div class="chart-wrapper" />'), _$canvas = $('<canvas id="myChart" />'), _$legend = $('<div class="legend-wrapper" />'), _barChart, _events = eventsInitializer.init(control, ['renderComplete', 'click']);
                ...
                function getChartJsDataSet(dataSet) {
                    ...
                    var rgbColor = colors.hexToRgb(dataSet.color), result = {
                        data: dataSet.data,
                        label: dataSet.label,
                    ...
                    _$canvas[0].onclick = function (evt) {
                        ...
                        if (activeBars.length) {
                            var eventData = {
                                ...
                                bars: collections.from(activeBars).select(function (chartJsElement) {
                                    return {
                ...
                control.addSeria = function (label, data) {
                    ...
                    collections.foreach(_barChart.datasets, function (dataset) {
                        //Chart.js bug - no highlights for newly added seria
                        var lastBar = dataset.bars[initData.labels.length - 1], prevBar = dataset.bars[initData.labels.length - 2];
