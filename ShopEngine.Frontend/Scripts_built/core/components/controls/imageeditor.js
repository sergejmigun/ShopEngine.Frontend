app.registerComponent('imageEditor', 'UI', [
    ...
    'Utils.strings',
    'Utils.math',
    function ($, promise, objects, eventsInitializer, collections, strings, math) {
        'use strict';
        return {
            init: function (container, initData) {
                var control = {}, _$wrapper = $('<div class="image-editor-container" />'), _$image, _cropState, _enableCropping, _currentState = {
                    rotation: 0,
                    originalWidth: 0,
                    originalHeight: 0,
                    currentWidth: 0,
                    currentHeight: 0,
                    zoom: 1
                }, _updateState, _events = eventsInitializer.init(control, ['onChange', 'onRemove', 'dblclick', 'submit']);
                function getButton(inner) {
                    var $button = $('<div class="round-button-circle" />'), $innerWrapper = $('<div class="round-button-inner" />');
                ...
                function loadImage($imageWrapper) {
                    return promise.create(function (success) {
                        _$image = $('<img />').attr('src', initData.imageUrl).load(function () {
                ...
                function initActions($actionsWrapper) {
                    ...
                    function initCropping() {
                        ...
                        appendBullet();
                        var $aspectRatioWrapper, $cropButton;
                        ...
                        if (initData.aspectRatio) {
                            var $buttons = [], setActiveButton = function ($activeButton) {
                            ...
                            collections.foreach(initData.aspectRatio, function (ratio) {
                                var $button;
                                if (ratio.free) {
                ...
                function initInfo($infoWrapper) {
                    var $originalWidth = $('<span />'), $originalHeight = $('<span />'), $currentHeight = $('<span />'), $currentWidth = $('<span />'), $zoomLevel = $('<span />'), $rotation = $('<span />'), $submit = $('<button class="btn btn-default">Save changes</button>');
                ...
                function init(success) {
                    var $imageWrapper = $('<div class="image-editor-wrapper" />'), $actionsWrapper = $('<div class="image-editor-actions" />'), $infoWrapper = $('<div class="image-editor-info" />');
