app.registerComponent('localizedInput', 'UI', [
    ...
    function ($, promise, objects, collections, shared, containerHelper, variableInput, localizationManagementService) {
        'use strict';
        return {
            init: function (container, initData) {
                var control = {}, _languages = [], _variableInput;
                function getLocalizedInputValue() {
                    var value = _variableInput.value();
                    if (!value) {
                        return null;
                    }
                    var items = collections.fromObject(value).select(function (variableItem) {
                        return {
                ...
                function getVariableInputValue(value) {
                    if (!value) {
                        return null;
                    }
                    var valueObj = {};
                ...
                function init(success) {
                    ...
                    localizationManagementService.findLanguages({}).then(function (languages) {
                        collections.foreach(languages.items, function (lang) {
                            _languages.push({
                                name: lang.shortName,
                ...
                control.displayError = function (error) {
                    var invalidLanguages = error.data;
