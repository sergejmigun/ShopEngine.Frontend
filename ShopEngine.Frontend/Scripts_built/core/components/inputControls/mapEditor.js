app.registerComponent('mapEditor', 'UI', [
    ...
    'Services.apiService',
    'UI.modals',
    function ($, promise, objects, collections, defaultInputValidationHandler, eventsInitializer, apiService, modals, numericBox, multiLineTextBox, htmlEditor) {
        'use strict';
        return {
            init: function (container, initData) {
                var control = {}, _$wrapper, _$mapContainer, _$addMarkerBtn, _$clearMarkersBtn, _value = {}, _markerModalHtml, _modalInst, _map, _markers = [], _validationHandler = defaultInputValidationHandler.init(control), _events = eventsInitializer.init(control, ['onChange', 'onRemove', 'init']);
                ...
                function openMarkerModal(modalTitle, $markerContent, modalButtons, lattitudeOptions, longtitudeOptions, titleOptions, captionOptions) {
                    return promise.create(function (success) {
                        modals.openCustom(modalTitle, $markerContent, modalButtons).then(function (modalInst) {
                            var markerLattitudeInit = numericBox.init($markerContent.find('.markerLattitude'), lattitudeOptions), markerLongtitudeInit = numericBox.init($markerContent.find('.markerLongtitude'), longtitudeOptions), markerTitleInit = multiLineTextBox.init($markerContent.find('.markerTitle'), titleOptions), markerCaptionInit = htmlEditor.init($markerContent.find('.markerCaption'), captionOptions);
                ...
                function openEditMarkerModal(html, markerData) {
                    var $markerContent = $(html), lattitudeOptions = getMarkerNumericOptions(markerData.lattitude), longtitudeOptions = getMarkerNumericOptions(markerData.longtitude), captionOptions = {}, titleOptions = {
                        rows: 2,
                        value: markerData.title
                    }, modalTitle = 'Edit marker', $saveButton = $('<button type="button" class="btn btn-success">Save</button>'), $removeMarkerButton = $('<button type="button" class="btn btn-danger pull-left">Delete marker</button>'), modalButtons = [$saveButton, $removeMarkerButton];
                    if (markerData.infowindow) {
                    ...
                    openMarkerModal(modalTitle, $markerContent, modalButtons, lattitudeOptions, longtitudeOptions, titleOptions, captionOptions)
                        ...
                        $saveButton.click(function () {
                            editMarker(res.markerLattitude.value(), res.markerLongtitude.value(), res.markerTitle.value(), res.markerCaption.value(), markerData);
                            res.modalInst.close();
                            _modalInst = null;
                        });
                ...
                function addMarker(lattitude, longtitude, title, caption, html) {
                    var latLng = toGmapPosition(lattitude, longtitude), markerData = {};
                    if (!latLng) {
                        latLng = _map.getCenter();
                    }
                ...
                function openAddMarkerModal(html) {
                    var $markerContent = $(html), lattitudeOptions = getMarkerNumericOptions(), longtitudeOptions = getMarkerNumericOptions(), captionOptions = {}, titleOptions = {
                        rows: 2
                    }, modalTitle = 'Add marker', $saveButton = $('<button type="button" class="btn btn-success">Save</button>'), modalButtons = [$saveButton];
                    openMarkerModal(modalTitle, $markerContent, modalButtons, lattitudeOptions, longtitudeOptions, titleOptions, captionOptions)
                        .then(function (res) {
                        $saveButton.click(function () {
                            addMarker(res.markerLattitude.value(), res.markerLongtitude.value(), res.markerTitle.value(), res.markerCaption.value(), html);
                            res.modalInst.close();
                        });
                    });
                }
                ...
                function init(success) {
                    ...
                    apiService.getTemplateHtml('mapEditorTemplate').then(function (html) {
                        ...
                        var mapCanvas = _$mapContainer[0], _mapOptions = {
