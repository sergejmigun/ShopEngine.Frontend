app.registerComponent('chainedSelectList', 'UI', [
    ...
    'Utils.strings',
    'UI.singleSelectList',
    function ($, promise, objects, defaultInputValidationHandler, eventsInitializer, containerHelper, apiService, collections, strings, singleSelectList) {
        'use strict';
        return {
            init: function (container, initData) {
                var control = {}, _$wrapper, _$listWrapper, _$readOnlyTextWrapper, _definedValues, _listsData = [], _settingValue, _validationHandler = defaultInputValidationHandler.init(control), _events = eventsInitializer.init(control, ['onChange', 'onRemove']);
                ...
                function getRelatedListItems(relatedListData, parentValue) {
                    return promise.create(function (success) {
                        ...
                        if (relatedListData.itemsUrl) {
                            var urlArgName = relatedListData.urlArgName || 'parent', url = strings.format('{0}?{1}={2}', relatedListData.itemsUrl, urlArgName, parentValue);
                ...
                function setListValue(listDataItem, value, index, completed) {
                    ...
                    if (listDataItem.relatedListData) {
                        getRelatedListItems(listDataItem.relatedListData, value).then(function (relatedListItems) {
                            var relatedListDataItem = _listsData[index + 1], relatedListValue = getListValueByIndex(index + 1, _definedValues);
                ...
                function initSelectList($el, items, url, relatedListData, index, nonSelectedText) {
                    function initRelatedData(inst, parentValue) {
                        if (!objects.isNullOrUndefined(inst.value())) {
                            var $relatedEl = appendListElement(), relatedItems, relatedItemsUrl;
                            if (relatedListData.getItems) {
                            ...
                            return initSelectList($relatedEl, relatedItems, relatedItemsUrl, relatedListData.relatedListData, index + 1, relatedListData.nonSelectedText);
                        }
                        return initSelectList(appendListElement(), [], null, relatedListData.relatedListData, index + 1, relatedListData.nonSelectedText);
                    }
                    return promise.create(function (success) {
                        var value = getListValueByIndex(index, initData.value), listDataItem = {};
                ...
                function init(success) {
                    ...
                    var initSelectListPromise = initSelectList(appendListElement(), initData.items, initData.itemsUrl, initData.relatedListData, 0, initData.nonSelectedText);
