app.registerComponent('gridEntityFinder', 'UI', [
    ...
    'UI.buttons',
    'Collections',
    function ($, promise, resources, objects, defaultInputValidationHandler, eventsInitializer, containerHelper, grid, finder, buttons, collections) {
        'use strict';
        return {
            init: function (container, initData) {
                var control = {}, _$wrapper = $('<div class="grid-entity-finder" />'), _$finderContainer, _$gridContainer, _columnsMap = {}, _grid, _finder, _readOnly, _disabled, _validationHandler = defaultInputValidationHandler.init(control), _events = eventsInitializer.init(control, ['onChange', 'onRemove']);
                ...
                function initFinder() {
                    return finder.init(containerHelper.appendTo(_$finderContainer, container.ready()), {
                        ...
                        finder.onChange(function () {
                            var valueData = finder.getValueData(), entity = valueData;
                            if (valueData) {
                                appendEntity(entity);
                                _events.onChange.invoke();
                            }
