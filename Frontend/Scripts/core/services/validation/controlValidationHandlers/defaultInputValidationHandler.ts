app.registerComponent('defaultInputValidationHandler', 'Services', [
    '$',
    'Collections',
    function ($: JQueryStatic, collections: ICollections) {
    'use strict';

    return {
        init: function (control) {
            var validationHandler = {} as Services.IDefaultInputValidationHandler,
                _$validationContainer: JQuery,
                _errorsData = [];

            function getMessage(error: UI.IValidationError) {
                return $('<span style="display: block;" />').text(error.message);
            }

            function displayError(error: UI.IValidationError, errorData) {
                if (!errorData) {
                    errorData = {
                        name: error.name
                    };
            ...
            function getErrorData(errorName: string) {
