    ...
    'use strict';
        ...
        return {
            init: function (inputsList, $validationContainer) {
                ...
                function displayItemErrors(error: Services.ValidationHandlers.IInputsListValidationError, errorData) {
                    var invalidControls = [];

                    collections.safeForeach(error.data, function (item) {
                        if (_controlsData[item.index]) {
                            var control = _controlsData[item.index],
                                controlError = {
                                    name: error.name,
                ...
                function displayError(error: Services.ValidationHandlers.IInputsListValidationError, errorData) {
                    if (!errorData) {
                        errorData = {
                            name: error.name,
                            invalidControls: []
                ...
                var _handler: Services.ValidationHandlers.IInputsListValidationHandler = {
        ...
        } as Services.ValidationHandlers.IInputsListValidationHandlerFactory;
}]);