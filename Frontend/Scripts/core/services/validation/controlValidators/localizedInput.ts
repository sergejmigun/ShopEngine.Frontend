app.registerComponent('localizedInput', 'Services.validation.controls', [
    ...
    function (promise: IPromise,
        ...
        var localizedInputValidation: Services.Validation.ILocalizedInput = {
            validate: function (control, validationFunc) {
                var itemsValidationResults = collections.from(control.value().items).select(function (localizedItem: any) {
                    return {
