app.registerComponent('backgroundProcessView', 'UI', [
    ...
    function ($: JQueryStatic,
        ...
        progressBar: UI.IProgressBarFactory,
        shared) {
        'use strict';

        return {
            init: function (container, initData) {
                var control = {} as UI.IBackgroundProcessView,
                    _$wrapper = $('<div class="bg-process-view" />'),
                    _currentProgressInfo: UI.IProgressInfo,
                    _rootProcessId: string,
                    _processData = {},
                    _events = eventsInitializer.init(control, ['requestCompleted', 'progressChanged', 'statusChanged'])
                ...
                function getProcessData(processId, processTitle, $wrapper) {
                    if (!_processData[processId]) {
                        var $processItemContainer = $('<div class="bg-process-item" />'),
                            $progressbarContainer = $('<div class="bg-process-item-bar" />'),
                            $resultMessageContainer = $('<div class="bg-process-item-result-message" />'),
                            $innerProcessContainer = $('<div class="bg-process-item-inner" />').hide();
                ...
                function displayInfo(progressInfo: UI.IProgressInfo, processTitle, $wrapper) {
                    ...
                    function renderInProgress() {
                        ...
                        if (progressInfo.innerProcess) {
                            processData.innerProces = displayInfo(
                                progressInfo.innerProcess.progressInfo,
                                progressInfo.innerProcess.title,
                                processData.innerProcessContainer
                            );
                        }
                    }
                ...
                function run() {
                    return apiService.get<UI.IProgressInfo>(initData.getProgressUrl).then(function (progressInfo) {
        ...
        } as UI.IBackgroundProcessViewFactory;
    }
]);