    ...
    function ($: JQueryStatic,
        ...
        return {
            init: function (container, initData) {
                var control = {} as UI.IEmotionsPicker,
                    _$wrapper = $('<div class="emotions-wrapper" />'),
                    _events = eventsInitializer.init(control, ['pick']);

                function setWrapperWidth(emotionsCount: number) {
                    var sideCount = Math.ceil(Math.sqrt(emotionsCount)),
                        totalWidth = (6 + initData.size) * sideCount;

                    _$wrapper.append('<div class="clear" />');
                    _$wrapper.css('width', totalWidth);
                }

                function appendEmotion(emotionName: string, path: string) {
                ...
                function initEmotion(emotionName: string, emotionSrc: string) {
                    var path = getEmotionPath(initData.size, emotionSrc);
                    appendEmotion(emotionName, path);
                }

                function initEmotions(emotions: string[]) {
                    var counter = 0;
            ...
            getEmotionHtml: function (emotionName, size) {
                ...
                var emotionSize = size,
                    path = getEmotionPath(emotionSize, _emotionsMap[emotionName]),
                    $emotionWrapper = getEmotionWrapper(emotionSize, path, emotionName);

                return dom.getOuterHtml($emotionWrapper);
            }
        } as UI.IEmotionsPickerFactory;
    }
]);