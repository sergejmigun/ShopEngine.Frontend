    ...
    function ($: JQueryStatic,
        ...
        return {
            init: function (container, initData) {
                var control = {} as UI.IAccordion,
                    _$wrapper = $('<ul class="-accordion" />'),
                    _panelWidth: number,
                    _events = eventsInitializer.init(control, ['expandingSection', 'expandedSection']);

                function setHorizontalAccordionPanelWidth($panelContent) {
                    $panelContent.css('width', _panelWidth + 'px');
                }

                function appendPanel(panel: UI.IAccordionPanel, index: number) {
                    var $panelWrapper = $('<li class="-accordion__panel" />'),
                        $panelHeading = $('<span class="-accordion__heading"><i class="-icon"></i></span>'),
                        $panelContent = $('<div class="-accordion__expander"></div>');
