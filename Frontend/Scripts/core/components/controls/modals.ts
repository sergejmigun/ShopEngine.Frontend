app.registerComponent('modals', 'UI', [
    ...
    function ($: JQueryStatic,
        ...
        function showModal($modalWrapper: JQuery, options?: UI.IModalOptions, onModalShow?: (modalObj: UI.IModal) => void) {
            var modalObj = {
                close: function () {
        ...
        return {
                ...
                function init(success) {
                    apiService.getTemplateHtml('modalTemplate').then(function (modalHtml) {
                        var modalObjects = createModalJQueryObjects(modalHtml),
                            $okButton = $('<button type="button" class="btn btn-primary" data-dismiss="modal">Ok</button>'),
                            templateInst,
                            onModalShow = eventsInitializer.initVoidEvent();
                        ...
                        if (confirmation.template) {
                            var $templateWrapper = $('<div />'),
                                tempalteData = confirmation.template as any;
            ...
            openSubmit: function (title, $content) {
                function init(success) {
                    apiService.getTemplateHtml('modalTemplate').then(function (modalHtml) {
                        var modalObjects = createModalJQueryObjects(modalHtml),
                            $submitButton = $('<button type="button" class="btn btn-primary" data-dismiss="modal">Submit</button>');
