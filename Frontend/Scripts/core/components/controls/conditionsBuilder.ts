app.registerComponent('conditionsBuilder', 'UI', [
    ...
    function ($: JQueryStatic,
        ...
        typedInput: UI.ITypedInputFactory) {
        'use strict';

        return {
            init: function (container, initData) {
                var control = {} as UI.IConditionsBuilder,
                    _$wrapper = $('<div class="conditions-builder" />'),
                    _data = initData.conditionsData || {} as UI.IConditionsData;

                function setNodata($container: JQuery) {
                    $container.append('<div class="no-data noData">No conditions added</div>');
                }

                function getConditionWrapper($container: JQuery) {
                ...
                function initCondition(condition: UI.IConditionsData, $container: JQuery) {
                    $container.find('>.noData').remove();

                    var $conditionWrapper = getConditionWrapper($container),
                        $row = $('<div class="row" />'),
                        $operandCol = $('<div class="col-md-2" />'),
                        $predicateActionCol = $('<div class="col-md-2" />'),
                        $predicateValueCol = $('<div class="col-md-7" />'),
                        $deleteButtonCol = $('<div class="col-md-1" />');

                    function initPredicateValue(predicate: UI.IConditionPredicate) {
                        function initControl() {
                            return predicate.initControl(containerHelper.appendTo($predicateValueCol, container.ready()));
                        }

                        function initTypedInput() {
                            var typeInitData = {} as any;
                    ...
                    function initPredicateAction(operand: UI.IConditionOperand) {
                ...
                function initConditionsGroup(group: UI.IConditionsData, $container: JQuery) {
                    $container.find('>.noData').remove();

                    var $conditionWrapper = getConditionWrapper($container),
                        $conditionBar = $('<div class="condition-bar" />'),
                        $conditionBody = $('<div class="condition-body" />');

                    $conditionWrapper.append($conditionBar).append($conditionBody);

                    function initConditionBar() {
                        var $conditionBarWrapper = $('<div class="row condition-bar-wrapper" />'),
                            $andOrCol = $('<div class="col-md-6" />'),
                            $buttonsCol = $('<div class="col-md-6" />');

                        function initAndOrSwitch() {
                            var $wrapper = $('<div class="btn-group" />'),
                                $and = $('<button type="button" class="btn w-50 btn-sm">And</button>'),
                                $or = $('<button type="button" class="btn w-50 btn-sm">Or</button>'),
                                activeCss = 'btn-primary',
                                inactiveCss = 'btn-default';
                        ...
                        function initButtons() {
                            var remove = buttons.getData('remove') as UI.IDropDownMenuButtonData,
                                addCondition = buttons.getData('add') as UI.IDropDownMenuButtonData,
                                addGroup = buttons.getData('add') as UI.IDropDownMenuButtonData;
        ...
        } as UI.IConditionsBuilderFactory;
    }
]);