    ...
    function ($: JQueryStatic,
        ...
        return {
            init: function (container, initData) {
                var control = {} as UI.IPagination,
                    _lastPage: number,
                    _targets = 5,
                    _$lists = [] as JQuery[],
                    _events = eventsInitializer.init(control, ['pageChanged']);

                function togglePagination(action: string) {
                ...
                function buildPagination() {
                    ...
                    function getListItem(page: number, html?: string) {
                        if (html === undefined) {
                            html = page.toString();
                        }
                    ...
                    function buildTarget() {
                        iterateLists(function ($list) {
                            var remainingLinksCount = _targets - 1,
                                startPage,
                                $currentListItem = getListItem(initData.page).attr('class', 'active');
