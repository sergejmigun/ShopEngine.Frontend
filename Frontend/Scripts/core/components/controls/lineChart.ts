    ...
    function ($: JQueryStatic,
        ...
        return {
            init: function (container, initData) {
                ...
                function getChartJsDataSet(dataSet: UI.ILineChartDataSet) {
                    if (!dataSet.color) {
                    ...
                    _$canvas[0].onclick = function (evt) {
                        ...
                        if (activePoints.length) {
                            var eventData = {
                                ...
                                points: collections.from(activePoints).select(function (chartJsElement: any) {
                                    return {
                ...
                control.addSeria = function (label, data) {
                    ...
                    collections.foreach(_lineChart.datasets as any[], function (dataset) {
                        // chart.js bug - no highlights for newly added seria
                        var lastPoint = dataset.points[initData.labels.length - 1],
                            prevPoint = dataset.points[initData.labels.length - 2];
