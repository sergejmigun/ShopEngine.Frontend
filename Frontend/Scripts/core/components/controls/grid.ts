    ...
    function ($: JQueryStatic,
        ...
        return {
            init: function (container, initData) {
                var _$wrapper = $('<div class="grid-wrapper" />'),
                    _$table: JQuery,
                    _$tbody: JQuery,
                    _$nodataTr: JQuery,
                    _$mainSelectionInput: JQuery,
                    _maxGroupingDetpth = 0;
                ...
                var _events = {
                    onOrderChanged: eventsInitializer.initEvent<{ column: string, direction: string }[]>(),
                    ...
                    onToggleDetails: eventsInitializer.initEvent<{ data: any, expanded: boolean }>()
                };

                var _control: UI.IGrid = {
                    ...
                    getSelectedRows: function () {
                        ...
                        function fillSelectedRows(rows: IRow[]) {
                ...
                function addAdditionalHeader(content?: string | JQuery, css?: string) {
                ...
                function selectRows(rowData) {
                    ...
                    if (rowData.innerRowsData) {
                        collections.foreach(rowData.innerRowsData as any[], function (innerRowData) {
                ...
                function selectionChanged(row: IRow) {
                    function deselectParents(row: IRow) {
                        if (row.parentRow) {
                ...
                function buildCells($tr: JQuery, row: IRow, cells: UI.IUiItemData[]) {
                    var renderPromises = [];
                ...
                function buildDetailsRow($tr: JQuery, details: UI.IGridRowDetailsInitData, row: IRow, groupingDepth: number) {
                    var $detailsTr = $('<tr class="grid-details-row" />'),
                        $detailsTd = $('<td class="grid-details-cell" />').attr('colspan', _columns.length),
                        $detailsButtonTd = $('<td class="grid-no-border-l grid-details-column" />'),
                        rendered = false;
                ...
                function buildDataRow($tr: JQuery, row: IRow, rowInitData: UI.IGridRowInitData, groupingDepth?: number) {
                    if (initData.settings.selectable) {
                ...
                function buildGroupingRow($groupingTr: JQuery, row: IRow, rowInitData: UI.IGridRowInitData, depth: number) {
                    function showInnerRows(row: IRow) {
                        if (!row.innerRows) {
                            return;
                        }
                    ...
                    function hideInnerRows(row: IRow) {
                        if (!row.innerRows) {
                            return;
                        }
                    ...
                    function toggleGrouping($groupingButtonTd: JQuery, row: IRow) {
                        if (row.groupCollapsed) {
                    ...
                    var buildPromises = [],
                        $groupingButtonTd = $('<td class="grid-group-button grid-no-border-side" />'),
                        $groupingTd = $('<td class="grid-no-border-l" />').attr('colspan', colspan);
                    ...
                    collections.foreach(rowInitData.innerRows, function (innerRowInitData) {
                        var buildRowResult = buildRow(innerRowInitData),
                            innerRow = buildRowResult.row;
                ...
                function buildNoDataRow() {
                    ...
                    var $td = $('<td />').attr('colspan', _columns.length),
                        noDataSetting = initData.settings.noData;
                ...
                function buildRow(rowInitData: UI.IGridRowInitData) {
                    var $tr = $('<tr />'),
                        row = {
                            element: $tr,
                ...
                function initColumnsOrdering(columnInner, columnData) {
                    ...
                    function toggleSortingData(colName, multiSorting) {
                        var result = [],
                            columnSortingData = initData.sortingData[colName];
                        ...
                        if (multiSorting) {
                            if (!columnSortingData.hasOwnProperty('index')) {
                                columnSortingData.index = collections.fromObject<any>(initData.sortingData).where(function (obj) {
                            ...
                            collections.foreach(initData.sortingData as {}, function (sortingDataItem: any, key) {
                                if (key === colName) {
                        ...
                        collections.fromObject<any>(initData.sortingData).where(function (obj) {
                            return obj.value.hasOwnProperty('index');
                    ...
                    function setOrderButtonContent(columnData) {
                        var $button = columnData.orderButton,
                            name = columnData.column.name;
                        ...
                        if (initData.sortingData[name]) {
                            ...
                            if (collections.fromObject<any>(initData.sortingData).count() > 1) {
                ...
                function buildDataColumns($thead: JQuery) {
                    var $tr = $('<tr />'),
                        buildColumnsPromises = [];

                    $thead.append($tr);

                    collections.foreach(initData.columns, function (column) {
                        var $th = $('<th />'),
                            $inner = $('<div class="grid-column-header" />'),
                            $innerContent = $('<div />'),
                            columnData = {
                                element: $th,
                                column: column,
