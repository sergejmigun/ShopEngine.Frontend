app.registerComponent('chatViewUiContext', 'Services.chatView', [
    ...
    function ($: JQueryStatic,
        ...
        scrollView: UI.IScrollViewFactory,
        multiLineTextBox: UI.IMultiLineTextBoxFactory) {
        'use strict';

        return {
            init: function (container, initData) {
                var context = {} as Services.ChatView.IChatViewUiContext,
                    _$wrapper = $('<div class="chat-view" />'),
                    _$messagesWrapper: JQuery,
                    _$boxWrapper: JQuery,
                    _$initialPageWrapper: JQuery,
                    _fixedArea: UI.IFixedArea,
                    _scrollView: UI.IScrollView,
                    _messageBox: UI.IMultiLineTextBox,
                    _submitButton: UI.IButton,
                    _events = eventsInitializer.init(context, ['onContentChanged', 'onMessageSubmit', 'onPageCleared']);

                function scrollTo($el: JQuery) {
                ...
                function appendDaySplit($container: JQuery, date: Date) {
                ...
                function initMessageActions($messageWrapper: JQuery, actions: UI.IDropDownMenuButtonData[], menuPosition: UI.DropDownMenuPostion) {
                ...
                function displayAuthorBar(message: UI.IChatViewMessageInitData) {
                ...
                function initMessageContent(
                    message: UI.IChatViewMessageInitData,
                    displayAuthor: boolean,
                    menuPosition: UI.DropDownMenuPostion) {
                    var $messageWrapper = $('<div class="chat-message" />'),
                        $messageInfo = $('<span class="message-info" />');
                ...
                function initMessage(message, prevMessage, nextMessage) {
                    ...
                    var lastMessageDate = prevMessage ? prevMessage.date : null,
                        lastAuthorId = prevMessage ? prevMessage.author.id : null,
                        daySplit = initData.splitByDays && !(dates.areSameDates(lastMessageDate, message.date)),
                        menuPosition = isRightPlacement ? UI.DropDownMenuPostion.right : UI.DropDownMenuPostion.left;
                ...
                function initMessages(messages: any[]) {
                ...
                function initInputBox() {
                    ...
                    var $input = $('<input type="hidden" />'),
                        $buttonsWrapper = $('<div class="button-wrapper" />');
                    ...
                    return promise.all<any>([inputBarTemplatePromise, messageBoxPromise]).then(function (results) {
                ...
                function init(success) {
                    ...
                    var inputBoxPromise = initInputBox(),
                        scrollViewPromise = initInfiniteScroll();
        ...
        } as Services.ChatView.IChatViewUiContextFactory;
    }
]);