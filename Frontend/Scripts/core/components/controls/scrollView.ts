app.registerComponent('scrollView', 'UI', [
    ...
    function (promise: IPromise,
        ...
        uiContext: Services.ScrollView.IScrollViewUiContextFactory) {
        'use strict';

        return {
            init: function (contentProvider, initData) {
                var control = {} as UI.IScrollView,
                    _hasOlderData,
                    _hasNewerData,
                    _pageContents = [],
                    _scrollViewUiContext,
                    _maxPages = initData.maxPages || 5,
                    _events = eventsInitializer.init(control, ['onContentChanged', 'onPageCleared']),
                    _controlContext;
                ...
                function initLoadMoreButton() {
                    var addNewerDataButton = _scrollViewUiContext.initLoadNewerDataButton(),
                        addOlderDataButton = _scrollViewUiContext.initLoadOlderDataButton();
